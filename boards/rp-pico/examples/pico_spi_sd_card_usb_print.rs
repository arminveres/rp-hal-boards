...
fn main() -> ! {
    ...
    serial
        .write(b"Initialize SPI SD/MMC data structures...")
        .unwrap();
    ...
    serial
        .write(b"Init SD card controller and retrieve card size...")
        .unwrap();
    ...
    serial.write(b"Getting Volume 0...").unwrap();
    ...
    serial.write(b"Root directory opened!").unwrap();
    // blink_signals(&mut led_pin, &mut delay, &BLINK_OK_LONG);
    ...
    if let Ok(mut file) = volume_mgr.open_file_in_dir(&mut volume, &dir, "O.TST", Mode::ReadOnly) {
        ...
        if read_count >= 2 {
            ...
            writeln!(text, "READ {} bytes: {:?}", read_count, buf).unwrap();
    ...
    if successful_read {
        serial
            .write(b"Successfully read previously written file 'O.TST'")
            .unwrap();
    } else {
        serial
            .write(b"Could not read file, which is ok for the first run.")
            .unwrap();
        serial.write(b"Reboot the pico!").unwrap();
    }
